/*
 * TinyAngband in the browser: terminal rendering, input and save persistence.
 * The game (tinyangband-core.js / .wasm) calls into Module.qb (see main-web.c).
 */
(function () {
	'use strict';

	var TILE = 16;                 /* source tile size in 16x16.png */
		var PERSIST = ['/tinyangband/lib/save', '/tinyangband/lib/user', '/tinyangband/lib/apex', '/tinyangband/lib/bone'];
	...
	function loadSoundCfg() {
		audio.cfg = {};
		var t = '';
		try { t = Module.FS.readFile('/tinyangband/lib/xtra/sound/sound.cfg', { encoding: 'utf8' }); } catch (e) { }
		t.split('\n').forEach(function (l) {
	...
	function removeSaves() {
		listFiles().forEach(function (p) {
			if (p.indexOf('/tinyangband/lib/save/') === 0) Module.FS.unlink(p);
		});
	}

	function saveFilePath() {
		var files = listFiles().filter(function (p) {
			return p.indexOf('/tinyangband/lib/save/') === 0 && !/\.(new|old)$/.test(p);
		});
		return files[0];
	}
	...
	window.Module = {
		...
		preRun: [function () {
			...
			['/tinyangband/lib/data', '/tinyangband/lib/info', '/tinyangband/lib/script'].forEach(function (d) { Module.FS.mkdirTree(d); });
			/* Own paths: IndexedDB names come from the mount points, shared per origin */
			Module.FS.chdir('/tinyangband');
			mountPersistent();
		}],
