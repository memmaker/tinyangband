...
/*
 * Browser (Emscripten/WASM) front end for TinyAngband.
 *
 * All drawing is done by JavaScript on one <canvas> per term (see
 * web/tinyangband.js).  Blocking input uses Asyncify: when the game waits
...
/* ---- JavaScript side (implemented in web/tinyangband.js) ---- */
...
void web_run_end(void)
{
	...
	if (k && suffix(k, "(?)")) { static char b[80]; strnfmt(b, sizeof b, "%s", k); b[strlen(b) - 3] = 0; k = b; }  /* hallucinating: effects.c */
	js_beacon("tinyangband", ev, player_name, k, dun_level, total_points(), (int)turn, p_ptr->lev);
}
