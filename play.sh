#!/bin/sh
# X11 frontend: 30x30 tiles (15x30 font + big-tile mode, -s = nearest-neighbour
# scaling) and extra windows: 1 inventory, 2 messages, 3 visible monsters +
# items, 4 recall. Screen 1440x932; XQuartz adds a ~28px title bar per window.
...
cd "$(dirname "$0")" && exec ./src/tinyangband -g -mx11 -- -n5 -b -s "$@"
